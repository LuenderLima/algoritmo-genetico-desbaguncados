package br.ufpb.es.ag.algorithm;
...
import br.ufpb.es.ag.entity.Dados;
import br.ufpb.es.ag.entity.Disciplina;
import br.ufpb.es.ag.entity.Horario;
import br.ufpb.es.ag.entity.Individuo;
import br.ufpb.es.ag.entity.Populacao;
import br.ufpb.es.ag.entity.Slot;

public class Algoritmo {

    private static double taxaDeCrossover;
    private static double taxaDeMutacao;
    private static List<Disciplina> tarefas = new ArrayList<Disciplina>();
    ...
    public static Individuo[] crossover(Individuo individuo1, Individuo individuo2) {
        ...
        List<Disciplina> tarefasNaoAlocadasFilho1 = getTarefas(); 
        List<Disciplina> tarefasNaoAlocadasFilho2 = getTarefas(); 
        ...
        for(i = pontoCorte2; i < getQuantidadeTarefas(); i++) {
        	slot = slotsPai1.get(i);
        	indiceDia = Dados.getIndiceDiaDaSemana(slot.getHorario().getDiaDaSemana());
     ...
     */
    public static boolean validaAlocacao(Disciplina tarefa, List<Disciplina> tarefasNaoAlocadas) {
    	for(Disciplina t: tarefasNaoAlocadas) {
    		if(tarefa.getNome().toUpperCase().equals(t.getNome().toUpperCase())) {   
    			tarefasNaoAlocadas.remove(t);
    			return true;
    		}
    	}
    	return false;
    ...
    public static List<Disciplina> getTarefas() {
    	List<Disciplina> listaTarefas = new ArrayList<Disciplina>();
    	for(Disciplina t: tarefas) {
    		for(int i=0; i < t.getFrequencia(); i++) {
    			listaTarefas.add(t);
    		}
    	}
		return listaTarefas;
	...
	public static void setTarefas(List<Disciplina> tarefas) {
		Algoritmo.tarefas = tarefas;
	}

	public static int getQuantidadeTarefas() {
		int quantidade = 0;
		for(Disciplina tarefa: tarefas) {
			quantidade += tarefa.getFrequencia();
		}
		return quantidade;
	}
